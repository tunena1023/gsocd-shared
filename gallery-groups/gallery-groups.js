/* ============================================================
   ...
   debajo del encabezado (visible sin necesidad de abrir la tarjeta),
   armado con lo que venga disponible en el group: bedrooms, bathrooms,
   division, supervisor -- cualquiera que falte simplemente no aparece
   (no truena, no deja un separador huerfano). "supervisor" es
   intencionalmente opcional por llamador: el portal de clientes
   (ordersgsocd.com) no lo manda a proposito -- no tiene sentido que
   un cliente vea que empleado interno tiene asignada su orden.
...
(function () {
  ...
  function render(containerId, groups, opts) {
    ...
    container.innerHTML = groups.map(function (g, gi) {
      ...
      /* El bloque de nombre + renglones de detalle ahora vive en
         gsocd-shared/order-card-header (19/09/2026) -- se comparte
         con Approvals/Review/Active/History/Schedule/Processing y
         demas listas de orden en los 3 portales, no solo Gallery.
         El contador de fotos y la flechita son especificos de
         Gallery (nadie mas los necesita), asi que van como
         trailingHtml -- el componente compartido no sabe de ellos. */
      ...
      var headerInner = window.GSOrderCardHeader
        ...
        /* Respaldo si por alguna razon order-card-header.js no cargo
           antes que este script -- nunca deberia pasar si el <script
           src> se agrega en el orden correcto, pero mejor no dejar
