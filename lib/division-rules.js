...
/* Texto listo para el campo Notes del renglon de historial -- mismo
   patron de "en palabras, no solo JSON crudo" que ya usa el resto del
   historial en los 3 repos. */
function divisionChangeNotes(result) {
  if (!result) return '';
  const names = result.causedBy.map(function (f) {
    return f.serviceName + ' (' + f.division + ')';
  }).join(', ');
  return 'Division changed from ' + (result.previousDivision || '(none)') + ' to Mixed because of: ' + names + '.';
}

module.exports = { resolveOrderDivision: resolveOrderDivision, divisionChangeNotes: divisionChangeNotes };
