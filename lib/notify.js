...
const OFFICE_TEMPLATES = {
  ...
  /* Inspeccion terminada (25/09/2026): el supervisor le dio "Inspection
     done" en Tech. La orden espera en Approvals a que la oficina revise
     lo que encontro y continue. */
  'inspection-done'(ctx) {
    const o = ctx.order;
    return {
      subject: `Inspection done: ${o.OrderID} by ${ctx.tech || 'supervisor'}${ctx.changes ? ' — service changes proposed' : ''}`,
      html: layout('Inspection done', ctx.changes ? 'The supervisor proposed service changes.' : 'Ready to schedule the work.',
        rowsTable([
          ['Order', o.OrderID],
          ['Client', `${o.BusinessName || o.Title || ''}${o.ClientID ? ' (' + o.ClientID + ')' : ''}`],
          ['Location', listHtml(locationLines(o).slice(1)), true],
          ['Supervisor', ctx.tech],
          ['Done', fmtDateTime(ctx.at)],
          ['Service changes', ctx.changes ? 'Yes — review them in Approvals' : 'None'],
          ['Suggested crew', (ctx.crew || []).join(', ')],
          ['Photos', ctx.photoCount ? ctx.photoCount + ' uploaded' : '']
        ]) + (ctx.notes ? quote(ctx.notes) : '') +
        button('Open in Approvals', `${ADMIN_URL}/admin.html?tab=approvals`, true),
        OFFICE_FOOTER)
    };
  },
