/* ============================================================
   ...
   VARIABLES DE ENTORNO (en Vercel, por proyecto):
     ...
     NOTIFY_FROM      buzon que manda (default orders@gsocd.com)
  ...
  received(ctx) {
    ...
    return {
      ...
      html: layout('Order received', hi ? `Thanks, ${hi}. We got your order.` : 'Thanks. We got your order.',
        ...
        button('View order', `${ORDERS_URL}/tracking.html`) +
  ...
  confirm(ctx) {
    ...
    return {
      ...
      html: layout('Action needed', 'Please confirm this change.',
        ...
        rowsTable(rows) +
        button('Review and confirm', `${ORDERS_URL}/tracking.html`, true) +
  ...
  completed(ctx) {
    ...
    return {
      ...
      html: layout('Completed', 'Your service is complete.',
        ...
        button('See photos', `${ORDERS_URL}/gallery.html`) +
...
const OFFICE_TEMPLATES = {
  ...
  'client-request'(ctx) {
    ...
    const tab = ctx.kind === 'new' ? 'approvals' : ctx.kind === 'confirmed' || ctx.kind === 'reactivation-confirmed' ? 'active' : 'review';
    const label = { new: 'New order', edit: 'Order edited', change: 'Change', cancel: 'Cancellation', reschedule: 'New dates', confirmed: 'Change confirmed', 'reactivation-confirmed': 'Reactivation confirmed' }[ctx.kind] || 'Request';
    ...
    return {
      ...
      html: layout('Client request', `${who} ${what}.`,
        ...
        button(tab === 'approvals' ? 'Open in Approvals' : tab === 'review' ? 'Open in Review' : 'Open in Admin', `${ADMIN_URL}/admin.html?tab=${tab}`, true),
        OFFICE_FOOTER)
    };
  },
...
async function notifyClient(g, opts) {
  ...
  try {
    ...
    await sendMail(g, cfg, { to: [recipient.email], subject, html: rendered.html, attachment });
...
async function sendClientIdEmail(g, opts) {
  ...
  try {
    ...
    subject = rendered.subject;
    await sendMail(g, cfg, { to: [to], subject, html: rendered.html });
