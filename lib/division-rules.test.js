const { resolveOrderDivision, divisionChangeHistoryPayload } = require('./division-rules');
...
// 2) Se agrega un servicio de otra division -> debe pasar a Mixed
{
  ...
  const payload = divisionChangeHistoryPayload(r);
  console.log('    payload: ' + JSON.stringify(payload));
  assert('payload trae division y causedBy', payload && payload.division === 'Mixed' && payload.causedBy.length === 1 && payload.causedBy[0].serviceName === 'Deck Staining');
}
